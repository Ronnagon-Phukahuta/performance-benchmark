...
def op1_read_csv() -> BenchmarkResult:
    ...
    with measure(f"{ENGINE}_op1_read_csv") as m:
        df = pl.scan_csv(RAW_CSV).collect(streaming=True)
...
def op5_join() -> BenchmarkResult:
    ...
    with measure(f"{ENGINE}_op5_join") as m:
        ...
        result = fact.join(dim, on="ticker_id", how="left").collect(streaming=True)
    print(f"  op5 done: {m.value.duration_sec:.2f}s | RAM: {m.value.peak_ram_mb:.1f}MB | Rows: {len(result):,}")
    return m.value


def op6_window() -> BenchmarkResult:
    # rolling_mean with over() is not supported in streaming mode —
    # requires the full group in memory. Falls back to eager collect.
    print(f"[{ENGINE}] Running op6_window (rolling 7-row avg close per ticker — streaming not supported, fallback to eager)...")
    with measure(f"{ENGINE}_op6_window") as m:
        df = pl.scan_csv(RAW_CSV).sort("date").collect(streaming=True)
...
def op9_concat() -> BenchmarkResult:
    # Collect streaming, split, lazy-concat with streaming.
    print(f"[{ENGINE}] Running op9_concat (split in half + lazy concat streaming)...")
    with measure(f"{ENGINE}_op9_concat") as m:
        df = pl.scan_csv(RAW_CSV).collect(streaming=True)
        half = len(df) // 2
        lf1 = df[:half].lazy()
        lf2 = df[half:].lazy()
        result = pl.concat([lf1, lf2]).collect(streaming=True)
