...
DUCKDB_PATH = os.path.join(DUCKDB_DIR, "batch_insert.db")
BATCH_SIZE = 10_000
...
def write():
    ...
    print("Running write benchmark (batch_insert)...")
    with measure("duckdb_batch_insert_write", data_path=DUCKDB_DIR) as m:
        con = duckdb.connect(DUCKDB_PATH)
        con.execute("DROP TABLE IF EXISTS stocks")
        con.execute(CREATE_TABLE_SQL)
        for i, start in enumerate(range(0, len(rows), BATCH_SIZE), start=1):
...
def read():
    ...
    with measure("duckdb_batch_insert_read", data_path=DUCKDB_DIR) as m:
        con = duckdb.connect(DUCKDB_PATH)
...
def query():
    ...
    with measure("duckdb_batch_insert_query", data_path=DUCKDB_DIR) as m:
        con = duckdb.connect(DUCKDB_PATH)
