...
DUCKDB_PATH = os.path.join(DUCKDB_DIR, "bulk_insert_polars.db")
...
def write():
    ...
    print("Running write benchmark (bulk_insert_polars)...")
    with measure("duckdb_bulk_insert_polars_write", data_path=DUCKDB_DIR) as m:
...
def read():
    ...
    with measure("duckdb_bulk_insert_polars_read", data_path=DUCKDB_DIR) as m:
        con = duckdb.connect(DUCKDB_PATH)
...
def query():
    ...
    with measure("duckdb_bulk_insert_polars_query", data_path=DUCKDB_DIR) as m:
        con = duckdb.connect(DUCKDB_PATH)
