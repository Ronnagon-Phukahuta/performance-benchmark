...
DUCKDB_PATH = os.path.join(DUCKDB_DIR, "bulk_insert_polars_arrow.db")
...
def write():
    ...
    bulk_df = df.select(["date", "ticker", "open", "high", "low", "close", "volume"])
    print(f"Loaded {len(bulk_df)} rows from CSV")
    print("Running write benchmark (bulk_insert_polars_arrow)...")
    with measure("duckdb_bulk_insert_polars_arrow_write", data_path=DUCKDB_DIR) as m:
        con = duckdb.connect(DUCKDB_PATH)
        con.execute("DROP TABLE IF EXISTS stocks")
        con.execute(CREATE_TABLE_SQL)
        con.register("df", bulk_df.to_arrow())
...
def read():
    ...
    with measure("duckdb_bulk_insert_polars_arrow_read", data_path=DUCKDB_DIR) as m:
        con = duckdb.connect(DUCKDB_PATH)
        df = con.execute("SELECT * FROM stocks").pl()
        con.close()
...
def query():
    ...
    with measure("duckdb_bulk_insert_polars_arrow_query", data_path=DUCKDB_DIR) as m:
        con = duckdb.connect(DUCKDB_PATH)
