...
def write():
    ...
    print("Running write benchmark (mongodb bulk_insert ordered=False)...")
    with measure("mongodb_bulk_insert_write", data_path="") as m:
        client, db = _db()
        db[COLLECTION].drop()
...
def read():
    ...
    with measure("mongodb_bulk_insert_read", data_path="") as m:
        client = MongoClient(MONGO_URI)
        db = client[DB_NAME]
...
def query():
    ...
    with measure("mongodb_bulk_insert_query", data_path="") as m:
        client = MongoClient(MONGO_URI)
        db = client[DB_NAME]
        pipeline = [
            {"$group": {
