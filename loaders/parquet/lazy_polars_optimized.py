...
def write():
    ...
    print("Running write benchmark (parquet_lazy_polars_optimized)...")
    with measure("parquet_lazy_polars_optimized_write", data_path=PARQUET_PATH) as m:
        df.write_parquet(PARQUET_PATH)
    print(f"Write done: {m.value.duration_sec:.2f}s | RAM: {m.value.peak_ram_mb:.1f}MB | Disk: {m.value.disk_size_mb:.1f}MB")


def read():
    print("Running read benchmark (lazy streaming)...")
    with measure("parquet_lazy_polars_optimized_read", data_path=PARQUET_PATH) as m:
...
def query():
    print("Running query benchmark (lazy streaming)...")
    with measure("parquet_lazy_polars_optimized_query", data_path=PARQUET_PATH) as m:
        result = (
