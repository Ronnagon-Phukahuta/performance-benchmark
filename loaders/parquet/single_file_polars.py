...
def write():
    ...
    print("Running write benchmark (parquet single_file_polars)...")
    with measure("parquet_single_file_polars_write", data_path=PARQUET_PATH) as m:
...
def read():
    ...
    with measure("parquet_single_file_polars_read", data_path=PARQUET_PATH) as m:
...
def query():
    ...
    with measure("parquet_single_file_polars_query", data_path=PARQUET_PATH) as m:
        df = pl.read_parquet(PARQUET_PATH, columns=["ticker", "close"])
